 ...
 */
export const yd_datetime_customFormat = (format = 'YYYY-MM-DD_HH-MM-SS') => {
    try {
        const now = new Date();
        ...
        // 使用相应的值替换格式中的占位符
        const formattedDateTime = format.replace('YYYY', year).replace('MM', month).replace('DD', day).replace('HH', hours).replace('MM', minutes).replace('SS', seconds);
