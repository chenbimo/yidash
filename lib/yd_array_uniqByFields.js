import { hashCode } from './yd_hash_generateHashCode'
import { serialize } from './yd_object_serialize'
/**
* 数组去重
...
*/
export const uniqueByFields = (array, fields) => {
    const uniqueMap = new Map()

    const resultArray = array.filter(item => {
        const uniqueKey = fields.map(field =>
            typeof item[field] === 'object' ? hashCode(serialize(item[field])) : item[field]
        ).join('|')
