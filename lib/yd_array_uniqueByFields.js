import { yd_hash_generateHashCode } from './yd_hash_generateHashCode'
...
export const yd_array_uniqueByFields = (array, fields) => {
    ...
    const resultArray = array.filter(item => {
        const uniqueKey = fields.map(field =>
            typeof item[field] === 'object' ? yd_hash_generateHashCode(yd_object_serialize(item[field])) : item[field]
        ).join('|')
