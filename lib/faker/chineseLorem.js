...
export default (len) => {
    ...
    for (let i = 1; i < len; i++) {
        if (Math.random() < 0.04) {
            ...
            if (simplifiedChinesePunctuationDB.indexOf(characters[i - 1]) === -1) {
                characters += simplifiedChinesePunctuationDB[getRandomNumber(simplifiedChinesePunctuationDB.length)];
                continue;
            }
        }
    ...
    if (simplifiedChinesePunctuationDB.indexOf(characters[characters.length - 1]) === -1) {
