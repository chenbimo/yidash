...
import { yd_object_serialize } from '../object/yd_object_serialize';
/**
 * 数组去重
...
export default (array, fields) => {
    ...
    const resultArray = array.filter((item) => {
        const uniqueKey = fields.map((field) => (typeof item[field] === 'object' ? yd_hash_getHashCode(yd_object_serialize(item[field])) : item[field])).join('|');
